from flask import Flask, Blueprint, redirect, url_for, render_template, request, flash, send_from_directory, send_file
from flask_sqlalchemy import SQLAlchemy
from .model import User, Dataset, Model
from ..database import db
from .neural_module import CreateModel, UseModel
from config import ALLOWED_EXTENSIONS, PATH_TO_UPLOAD, COLUMNS, HIDDEN_LAYER_OPTIONS, ACTIVATION_OPTIONS, UPLOAD_FOLDER
...
@neural.route("/home", methods=["GET", "POST"])
@neural.route("/", methods=["GET", "POST"])
def home_page():
...
gl_model_name = ""
@neural.route("/create_model", methods = ["GET", "POST"])
def create_model():
    ...
    if request.method == "POST" and not model_training:
        ...
        if dataset_status == "Chosen" or dataset_status == "Uploaded":
            ...
            while i < number_layers and correct_fields:
                ...
                i += 1
            #print("Chosen hidden layer:", request.form["choose-hidden-layer"])
            #print("Hidden units:", request.form["hidden_units"])
            #print("Chosen activation:", request.form["choose-activation"])
            print("Chosen hidden layers:", hidden_layers)
            print("Hidden units:", hidden_units)
            print("Chosen activation:", activation_functions)
            print()
            model_training = True
            #model = CreateModel(os.path.join(UPLOAD_FOLDER, dataset_to_train.filePath), 
            #int(request.form["input_units"]), request.form["choose-hidden-layer"], 
            #int(request.form["hidden_units"]), request.form["choose-activation"])
            ...
            # model, model_history, test_scores, time = CreateModel(os.path.join(PATH_TO_UPLOAD, current_user.directory, "datasets", dataset_to_train.name, 
            # dataset_to_train.filePath), dataset_settings,
            # int(request.form["input_units"]), hidden_layers, 
            # hidden_units, activation_functions)
            ...
            #save_model_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), app.config['UPLOAD_FOLDER'], 
            #    current_user.directory, model_name)
            ...
            #dataset = Dataset.query.filter_by(id=dataset_to_train_id).first()
            print(save_model_path)
            #model_struct = ModelStructure(request.form["input_units"], hidden_layers, hidden_units, activation_functions)
            ...
            #model_db = Model(current_user.id, dataset_to_train.id, model_name, model_name, model_struct)
            ...
            if (to_predict):
                ...
                #df_predict = pd.read_excel(os.path.join(PATH_TO_UPLOAD, current_user.directory, "validations", 
                #model_name + ".xlsx"))
            ...
            # if (request.form["dataset-settings-2"] == "Yes"):
            #     dataset_settings["sheet"] = 2
            #     # TO DO:
            #     #dataset_settings["prediction_column"] = request.form["choose-prediction"]
            #     dataset_settings["prediction_column"] = "A"
            #     print("Validating from create_model. Column to predict:", dataset_settings["prediction_column"])
            #     validatePath = os.path.join(PATH_TO_UPLOAD, current_user.directory, "validations", 
            #     model_db.name + ".xlsx")
            #     print("Путь сохранения результата:", validatePath)
            #     try:
            #         Validate(model, input_units,
            #         os.path.join(PATH_TO_UPLOAD, current_user.directory, "datasets", dataset_to_train.name, dataset_to_train.filePath), 
            #         validatePath, dataset_settings, list_factors, mean, std)
            #         model_training = False
            #     except Exception as ex:
            #         model_training = False
            #         gl_model_name = ""
            #         print(ex.args[0])
            #         return ex.args[0]
            #     #return send_file(validatePath, as_attachment=True)
...
@neural.route("/use_model", methods = ["GET", "POST"])
def use_model():
    global model_training
    if request.method == "POST" and not model_training:
        current_user = User.query.filter_by(id=1).first()
...
@neural.route("/page_not_found")
def page_not_found():
    return "<h1>Ошибка: данной страницы не существует</h1>"

@neural.errorhandler(404)
def wrong_page(page):
